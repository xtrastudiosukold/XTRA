! function(modules) {
    ...
    const server_server = new server(function() {
        const config = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "config.json")),
            configFromString = function() {
                const connectionString = GetConvar("mysql_connection_string", "mysql://root@localhost/xtra");
                let cfg = {};
    ...
    }, !0), onNet("xtra:request-data", () => {
        const src = source;
        emitNet("xtra:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("xtra:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("xtra:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("xtra:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("xtra:update-status", src, data)
        }, null, "xtra").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("xtra:update-variables", src, data)
        }, null, "xtra").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
