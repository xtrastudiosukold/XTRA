local cfg = {}
local cfgRP = module("cfg/cfg_rpemotes")
...
cfg.guns = {
    ...
    ["holdingvest3"] = {"export@holdingvest", "head_000_r", "Holding Vest 3", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
...
local function doesEmoteWithNameExist(name)
    if cfg.emotes[name] or cfg.dances[name] or cfg.custom[name] or cfg.props[name] or cfg.guns[name] then
        return true
    else
        return false
    end
...
local function canExternalEmoteBeUsed(emoteInfo)
    ...
    if emoteInfo.AnimationOptions then
        if emoteInfo.AnimationOptions.Prop then
            if not IsModelValid(GetHashKey(emoteInfo.AnimationOptions.Prop)) then
                return false
            end
        end
        if emoteInfo.AnimationOptions.SecondProp then
            if not IsModelValid(GetHashKey(emoteInfo.AnimationOptions.SecondProp)) then
                return false
            end
        end
    end
...
local function canEmoteNameBeUsed(name)
    return not string.match(name, "shield")
end
...
local function copyExternalEmote(emoteInfo)
    ...
    if emoteInfo.AnimationOptions then
        newEmoteInfo.animationOptions = {
            emoteMoving = emoteInfo.AnimationOptions.EmoteMoving,
            emoteDuration = emoteInfo.AnimationOptions.EmoteDuration,
            emoteLoop = emoteInfo.AnimationOptions.EmoteLoop,
            syncOffsetFront = emoteInfo.AnimationOptions.SyncOffsetFront,
            prop = emoteInfo.AnimationOptions.Prop,
            propBone = emoteInfo.AnimationOptions.PropBone,
            propPlacement = emoteInfo.AnimationOptions.PropPlacement,
            secondProp = emoteInfo.AnimationOptions.SecondProp,
            secondPropBone = emoteInfo.AnimationOptions.SecondPropBone,
            secondPropPlacement = emoteInfo.AnimationOptions.SecondPropPlacement,
        }
    end
    return newEmoteInfo
end
