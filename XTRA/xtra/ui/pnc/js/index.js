...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://xtra/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://xtra/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://xtra/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://xtra/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[XTRA PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://xtra/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[XTRA PNC] Full name needs to be provided")
                }else{
                    $.post('https://xtra/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://xtra/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://xtra/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://xtra/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://xtra/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://xtra/savenotes', JSON.stringify({
            ...
            getPlateImage(plate) {
                $.get(`https://api.arthur.gg/xtraimage/plate?plate=${plate}`)
                return `https://api.arthur.gg/xtraimage/plate?plate=${plate}`
            },
            getTaxDisc(name, plate, id) {
                $.get(`https://api.arthur.gg/xtraimage/tax?plate=${plate}&name=${name}&id=${id}`)
                return `https://api.arthur.gg/xtraimage/tax?plate=${plate}&name=${name}&id=${id}`
            },
            savePersonNotes() {
                $.post("https://xtra/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://xtra/generatewarrant", JSON.stringify({}))
            }
            },
