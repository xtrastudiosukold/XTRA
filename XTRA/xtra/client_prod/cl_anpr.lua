    ...
    function(o, p)
        if c ~= "" then
            ...
            if table.count(p) ~= 0 then
                q = table.concat(p, " ")
                TriggerServerEvent("XTRA:flagVehicleAnpr", string.upper(c), q)
            else
                tXTRA.notify("Error~w~: You must provide a valid flag reason.")
            end
        end
    end
)
RegisterCommand(
    ...
    function(o, p)
        if d ~= "" then
            ...
            if table.count(p) ~= 0 then
                q = table.concat(p, " ")
                TriggerServerEvent("XTRA:flagVehicleAnpr", string.upper(d), q)
            else
                tXTRA.notify("Error~w~: You must provide a valid flag reason.")
            end
        end
...
tXTRA.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(tXTRA.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tXTRA.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
                SetTimeout(
                    10000,
                    function()
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(tXTRA.getPlayerPed(), false) then
        if GetVehicleClass(tXTRA.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
