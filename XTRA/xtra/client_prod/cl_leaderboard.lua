local a = {
    ...
    rows = {
        {"ker", "BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"dbi", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Serio", "RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"ker", "BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"dbi", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Serio", "RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"ker", "BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"dbi", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Serio", "RED TEAM", "", false, 5, 2, 0, "0.5"}
    }
...
RegisterCommand(
    ...
    function()
        if tXTRA.isDev() then
            local b = tXTRA.createLeaderboard(a)
            Wait(1000)
            tXTRA.closeLeaderboard(b)
        end
    end,
    false
)
...
function tXTRA.closeLeaderboard(b)
    c[b] = false
end
function tXTRA.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"XTRA Event"})
...
function tXTRA.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: XTRA:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "XTRA:updateScoreboard",
    function(u, v, w)
        tXTRA.updateScoreboard(u, v, w)
    end
)
function tXTRA.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tXTRA.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tXTRA.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tXTRA.removePlayerFromLeaderboard(u)
