...
RMenu.Add(
    "scubadiving",
    "main",
    RageUI.CreateMenu("", "", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight(), "xtra_scubajob", "xtra_scubajob")
)
RMenu:Get("scubadiving", "main"):SetSubtitle("XTRA | ~b~Scuba Diving")
local function j()
...
AddEventHandler(
    ...
    function(l, m)
        if m then
            ...
            tXTRA.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
            tXTRA.addMarker(
                a.jobPosition.x,
                a.jobPosition.y,
                a.jobPosition.z - 0.2,
            ...
            tXTRA.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
        end
    end
)
RageUI.CreateWhile(
                ...
                function()
                    if not e then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("XTRA:requestScubaJob")
                                end
                            end
                        )
                    end
                    if d ~= 0 then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("XTRA:claimScubaReward", c)
                                    d = 0
                                    c = nil
                                    e = false
                                    i = false
                                    ...
                                    tXTRA.setCustomization(h)
                                    local x = PlayerPedId()
                                    SetEnableScuba(x, false)
                                    SetPedMaxTimeUnderwater(x, 10.0)
                                    tXTRA.setMaxUnderWaterUITimenewTime(10.0)
                                end
                            end
                        )
                    end
...
local function y()
    for z, A in pairs(b) do
        tXTRA.removeMarker(A.objectMarker)
        tXTRA.removeArea(A.objectArea)
...
function startScubaDiving()
    tXTRA.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tXTRA.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("XTRA:finishScubaJob", c, d)
end
RegisterNetEvent(
    "XTRA:createScubaJob",
    function(J, K)
        c = K
        e = true
        tXTRA.disableCustomizationSave(e)
        DoScreenFadeOut(500)
        Wait(600)
        ...
        SetBlipAlpha(L, 180)
        local q = function(r)
            tXTRA.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 47) then
                ...
                local M = tXTRA.getPlayerVehicle()
                if M ~= 0 then
                    if GetEntityModel(M) ~= GetHashKey("dinghy") then
                        tXTRA.notify("You must be in a dinghy to start scuba diving!")
                    else
                        tXTRA.removeArea("scubajob_" .. J.name)
                        f = "~y~Search for treasures and return to your boat when finished"
                        createRewardObjects(J)
                        CreateThread(startScubaDiving)
                    end
                else
                    tXTRA.notify("You must be in a boat to start scuba diving!")
                end
            end
        end
        SetWaypointOff()
        local N = tXTRA.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
        SetNewWaypoint(J.position.x, J.position.y)
        tXTRA.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
        h = tXTRA.getCustomization()
        local p = math.random(1, 15)
        local O = PlayerPedId()
        ...
        g =
            XTRA.spawnVehicle(
            "dinghy",
            J.dinghySpawnPositions[P].x,
            J.dinghySpawnPositions[P].y,
            J.dinghySpawnPositions[P].z,
            J.dinghySpawnHeading,
            true,
            true,
            true
        )
        ...
        TriggerServerEvent("XTRA:scubaSetVehicle", c, R)
        DoScreenFadeIn(2000)
        SetPedScubaGearVariation(PlayerPedId())
        Citizen.CreateThread(
            function()
                while e do
                    if i then
                        drawPlaneScaleForm("MISSION FAILED", "Your boat has been destroyed, go get a new one!")
                        SetWaypointOff()
                        TriggerServerEvent("XTRA:finishScubaJob", c, 0)
                        TriggerServerEvent("XTRA:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
...
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tXTRA.loadModel(A.objectModel)
        ...
        local q = function(r)
            tXTRA.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tXTRA.notify("~g~Item collected.")
                d = d + 1
                tXTRA.removeMarker(b[r.objectId].objectMarker)
                tXTRA.removeArea(b[r.objectId].objectArea)
        ...
        tXTRA.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
