    ...
    function(D, E)
        if E then
            local F = function()
                ...
                c = true
                i = deepcopy(h)
                j = tXTRA.getCustomization()
                RageUI.Visible(RMenu:Get("XTRAtattoos", "mainMenu"), true)
            end
            local G = function()
                ...
                RageUI.Visible(RMenu:Get("XTRAtattoos", "mainMenu"), false)
                tXTRA.setCustomization(j)
                j = nil
                ClearPedDecorations(tXTRA.getPlayerPed())
                for H = 1, #i, 1 do
                    AddPedDecorationFromHashes(
                        tXTRA.getPlayerPed(),
                        GetHashKey(i[H].collection),
                        GetHashKey(e[i[H].collection][i[H].current].nameHash)
                    )
                end
                c = false
                h = {}
            ...
            local I = function()
                ...
                if RageUI.Visible(RMenu:Get("XTRAtattoos", "mainMenu")) then
            ...
            for B, J in pairs(f) do
                ...
                Wait(100)
                tXTRA.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
                tXTRA.addBlip(J.x, J.y, J.z, 75, 0, "Tattoo Parlour")
                tXTRA.addMarker(
                    J.x,
                    J.y,
                    J.z,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Wait(500)
            if tXTRA.isInComa() and not P then
                P = true
            end
            if not tXTRA.isInComa() and P then
                Wait(5000)
                TriggerServerEvent("XTRA:getPlayerTattoos")
                P = false
            end
...
RegisterNetEvent(
    ...
    function(Q)
        ...
        for H = 1, #R, 1 do
            ...
            AddPedDecorationFromHashes(
                tXTRA.getPlayerPed(),
                GetHashKey(R[H].collection),
                GetHashKey(e[R[H].collection][R[H].current].nameHash)
            )
            table.insert(h, R[H])
        end
    end
            ...
            RageUI.DrawContent(
                ...
                function()
                    for H = 1, #d, 1 do
                        ...
                        RageUI.List(
                            T.name,
                            get_num_tattoos(T.value),
                            d[H].currentListIndex,
                            "Tattoo Add/Removal Price: £1,000",
                            {},
                            true,
                            function(U, V, W, X)
                                if V then
                                    if
                                        IsControlJustPressed(0, 177) or IsControlJustPressed(0, 194) or
                                    ...
                                    if c and not l and IsControlJustReleased(0, 178) then
                                        ...
                                        TriggerServerEvent("XTRA:saveTattoos", i, 1000)
                                    end
                                end
                                if W then
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(tXTRA.getPlayerPed(), 297.7296)
    local C = tXTRA.getPlayerPed()
    local a2 = false
    if _ ~= nil and a0 ~= nil and a1 ~= nil then
        if p then
            ...
            if GetEntityModel(tXTRA.getPlayerPed()) == -1667301416 then
                tXTRA.loadCustomisationPreset("TattooMale")
            else
                tXTRA.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(tXTRA.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(
            ...
            AddPedDecorationFromHashes(tXTRA.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(tXTRA.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(tXTRA.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(tXTRA.getPlayerPed()))
