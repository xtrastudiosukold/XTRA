...
local g = 0
local h = false
local i = {GetHashKey("WEAPON_PDGLOCK"), GetHashKey("WEAPON_STUNGUN"), GetHashKey("WEAPON_NIGHTSTICK")}
function notify(j)
            ...
            if c then
                if IsControlJustPressed(0, 73) then
                    ...
                    timeout(
                        k,
                        string.format(
                            "Do not press X to cancel the animation! Your weapons have been holstered for %ss",
                            k / 1000
                        )
                    )
                end
                        ...
                        if o ~= GetHashKey("WEAPON_UNARMED") then
                            if table.has(i, o) then
                                ...
                                if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
                                    timeout(
                                        3500,
                                        "Do not change weapons to equip quicker! Your weapons have been holstered for 3.5s"
                                    )
                                else
                                    a = false
                                    if o == "WEAPON_PDGLOCK" and GetPedDrawableVariation(n, 7) == 187 then
                                        tXTRA.loadCustomisationPreset("TaserInHolster")
                                    end
                                    if not d then
                                        a = false
                                        b = false
                            ...
                            if table.has(i, f) then
                                ...
                                if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
                                    ...
                                    if h and GetPedDrawableVariation(n, 7) == 161 then
                                        tXTRA.loadCustomisationPreset("TaserOutHolster")
                                    end
                                end
                            else
                                loadAnimDict("reaction@intimidation@1h")
                                ...
                                RemoveAnimDict("reaction@intimidation@1h")
                                b = true
                                tXTRA.setWeapon(n, GetHashKey(f), true)
                                Citizen.Wait(1300)
                                tXTRA.setWeapon(n, "WEAPON_UNARMED", true)
                                Citizen.Wait(700)
                                ClearPedTasks(n)
                                a = true
                                b = false
...
function tXTRA.setWeapon(m, r, s)
