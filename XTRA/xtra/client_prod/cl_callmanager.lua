...
RegisterNetEvent(
    "XTRA:addEmergencyCall",
    function(o, p, q, r, s, t)
        local u = 0
        if t == "admin" and tXTRA.getStaffLevel() > 0 then
            tXTRA.notify("~b~Admin ticket received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tXTRA.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            e[o] = r
        elseif t == "met" and tXTRA.globalOnPoliceDuty() then
            tXTRA.notify("~b~MET Police call received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tXTRA.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            e[o] = r
        elseif t == "nhs" and tXTRA.globalNHSOnDuty() then
            tXTRA.notify("~b~NHS call received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tXTRA.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
    ...
    function()
        while true do
            if a then
                ...
                DrawRect(0.251, 0.528, 0.485, 0.056, 0, 0, 0, 225)
                SetScriptGfxDrawOrder(0.0)
                DrawRect(0.251, 0.559, 0.485, 0.0059999999999999, o.r, o.g, o.b, 150)
                SetScriptGfxDrawOrder(0.0)
                ...
                if CursorInArea(0.4234, 0.4614, 0.9444, 0.9759) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                        if h ~= nil then
                            TriggerServerEvent("XTRA:TakeTicket", h)
                            a = not a
                            ...
                            if C ~= nil then
                                TriggerServerEvent("XTRA:TakeTicket", C)
                                a = not a
                                SetNewWaypoint(e[h].x, e[h].y)
                                TriggerEvent("XTRA:showHUD", true)
                                globalHideEmergencyCallUI = false
                                ...
                                tXTRA.notify("~r~No calls available.")
                            end
                        end
                    end
                else
                ...
                -- The following 'then' keyword seems out of place, please review the logic.
                -- then
...
local function u()
    if IsUsingKeyboard(2) and (tXTRA.getStaffLevel() > 0 or globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty) then
        a = not a
        if not a then
            TriggerEvent("XTRA:showHUD", true)
            globalHideEmergencyCallUI = false
...
RegisterCommand("callmanager", u, false)
RegisterKeyMapping("callmanager", "Open Call Manager", "KEYBOARD", "OEM_3")
Citizen.CreateThread(
    function()
        while true do
