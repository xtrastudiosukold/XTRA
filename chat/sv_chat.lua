...
local function routeMessage(source, author, message, mode, fromConsole)
    if source >= 1 then
        author = exports["xtra"]:GetDiscordName(source)
    end
    ...
    if mode == "OOC" then
        TriggerEvent("XTRA:ooc", source, message)
        return
    end
    if mode == "Gang" then 
    ...
    if mode == "Anonymous" then
        TriggerEvent("XTRA:Anon", source, message)
        return
    end
    if mode == "Police" then
    ...
    if mode == "Admin" then
        for k,v in ipairs(GetPlayers()) do
            if exports['xtra']:hasPermission(v, "admin.tickets") then
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = exports["xtra"]:GetDiscordName(source)
