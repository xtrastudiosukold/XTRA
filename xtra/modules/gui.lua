...
function XTRA.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  XTRAclient.openMenuData(source,{menudata})
end

-- force close player menu
function XTRA.closeMenu(source)
  XTRAclient.closeMenu(source,{})
end
...
function XTRA.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  XTRAclient.prompt(source,{title,default_text})
end
...
function XTRA.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  XTRAclient.request(source,{id,text,time}) -- send request to client
...
function XTRA.registerMenuBuilder(name, builder)
...
function XTRA.buildMenu(name, data, cbr)
...
function XTRA.openMainMenu(source)
  XTRA.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    XTRA.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tXTRA.closeMenu(id)
...
function tXTRA.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tXTRA.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tXTRA.requestResult(id,ok)
...
function tXTRA.openMainMenu()
  XTRA.openMainMenu(source)
end
...
function XTRA.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for XTRA.addStaticMenuChoices calls
...
AddEventHandler("XTRA:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    XTRAclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
