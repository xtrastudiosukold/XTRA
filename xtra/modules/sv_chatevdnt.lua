...
Citizen.CreateThread(function()
    while true do
        if over then
            ...
            theString = Code
            --TriggerClientEvent('chatMessage', -1, "XTRA","^2[Mini-Event] Who writes the word: " .. theString .. " first gets £" .. getMoneyStringFormatted(theReward))
            TriggerClientEvent('chatMessage', -1, "XTRA",  { 255, 255, 255 }, "^2[Mini-Event] Who writes the word: " .. theString .. " first gets £" .. getMoneyStringFormatted(theReward),  "ooc", "OOC")
            SetTimeout(10000, function()
                if not over then
                    TriggerClientEvent('chatMessage', -1, "XTRA",  { 255, 255, 255 }, "^2[Mini-Event] Time is over, no one wrote the word!",  "ooc", "OOC")
                    over = true
                    theReward = 0
                    theString = ""
                end
...
function HandleMoneys(user_id, theReward) 
    TriggerClientEvent('chatMessage', -1, "XTRA",  { 255, 255, 255 }, "^2[Mini-Event] " .. XTRA.GetPlayerName(user_id) .. " wrote the word and won £" .. getMoneyStringFormatted(theReward),  "ooc", "OOC")
    -- Reset Event
    theReward = 0
    theString = 0 
    over = true;
