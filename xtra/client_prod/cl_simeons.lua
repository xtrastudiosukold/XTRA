...
RegisterNetEvent(
    "XTRA:gotCarDealerInstances",
    function(m)
        a = m
    end
)
RegisterNetEvent(
    "XTRA:gotCarDealerCategories",
    function(n)
        b = n
    end
)
RegisterNetEvent(
    "XTRA:carDealerPurchased",
    function()
        DeleteVehicle(f)
        f = 0
        g = 0
        ...
        SetEntityCoords(tXTRA.getPlayerPed(), o.x, o.y, o.z)
    end
)
local function p(q)
    if q ~= nil then
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tXTRA.getPlayerPed()), false, false)
        ...
        TaskWarpPedIntoVehicle(tXTRA.getPlayerPed(), z, -1)
        ...
        tXTRA.notify("Could not load vehicle")
        return -1
    end
end
CreateThread(
    function()
        for A, B in pairs(l.simeonsInstances) do
            local C, D, E, F, G, H, I =
                ...
                B.permissionsTable,
                B.categorys
            tXTRA.addBlip(C.x, C.y, C.z, D, E)
            tXTRA.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
        end
    end
)
Citizen.CreateThread(
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tXTRA.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
            ...
            if a ~= nil then
                for A, B in pairs(a) do
                    local C, D, E, F, G, H, I =
                        ...
                        B.permissionsTable,
                        B.categorys
                    c[A] = #(GetEntityCoords(tXTRA.getPlayerPed()) - C)
                end
            end
            Wait(250)
        end
                                            ...
                                            for Q, R in pairs(b[M]) do
                                                if Q ~= "_config" then
                                                    RageUI.ButtonWithStyle(
                                                        R[1],
                                                        "Boot size (" .. R[3] .. "kg)",
                                                        {RightLabel = "£" .. getMoneyStringFormatted(R[2])},
                                                        true,
                                                        function(N, O, P)
                                                            if O then
                                                                if f == 0 or g ~= Q then
                ...
                function()
                    if d ~= nil then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(N, O, P)
                                if P then
                                    DeleteVehicle(GetVehiclePedIsIn(tXTRA.getPlayerPed()))
                                    if not k then
                                        RageUI.ActuallyCloseAll()
                                        hash = GetHashKey(j)
                                        ...
                                        if HasModelLoaded(hash) then
                                            ...
                                            testDriveSeconds = 60
                                            local S = GetEntityCoords(tXTRA.getPlayerPed())
                                            DecorSetInt(testDriveCar, decor, 945)
                                            SetModelAsNoLongerNeeded(hash)
                                            TaskWarpPedIntoVehicle(tXTRA.getPlayerPed(), testDriveCar, -1)
                                            setVehicleFuel(testDriveCar, 100)
                                            tXTRA.notify("~g~You have 1 minute to test drive this vehicle!")
                                            for T = 0, 24 do
                                            ...
                                            SetTimeout(
                                                ...
                                                function()
                                                    if k then
                                                        ...
                                                        SetEntityCoords(tXTRA.getPlayerPed(), S.x, S.y, S.z)
                                                        tXTRA.notify("Test drive over!")
                                                        p()
                                                    end
                                                end
                                            )
                                            ...
                                            Citizen.CreateThread(
                                                function()
                                                    while k do
                                                        local U = GetVehiclePedIsIn(tXTRA.getPlayerPed(), false)
                                                        if U ~= nil then
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.ButtonWithStyle(
                        ...
                        function(N, O, P)
                            if P then
                                TriggerServerEvent("XTRA:purchaseCarDealerVehicle", e, j)
                                TriggerServerEvent("XTRA:FetchCars", "Standard Garage")
                            end
                        end,
                        RMenu:Get("cardealer", "mainmenu")
                    )
...
tXTRA.createThreadOnTick(func_previewSimeonsVehicle)
