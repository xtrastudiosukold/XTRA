...
RegisterNetEvent(
    "XTRA:flashbangExplode",
    function(coords)
        if #(coords - tXTRA.getPlayerCoords()) <= b then
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tXTRA.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tXTRA.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(
                function()
                    ...
                    if e ~= 0 then
                        ...
                        AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                        TriggerServerEvent("XTRA:flashbangThrown", f)
                    end
                end
            )
            SetTimeout(
...
tXTRA.createThreadOnTick(func_checkForFlashbang)
