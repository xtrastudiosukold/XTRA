...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("XTRA:toggleAmbulanceDoors", tXTRA.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        tXTRA.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
    ...
    function()
        ...
        while true do
            ...
            if d ~= nil and d ~= 0 then
                ...
                if
                    not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and
                        tXTRA.getPlayerVehicle() == 0
                 then
                    if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                        if GetVehicleDoorAngleRatio(c, 4) ~= 0 then
                            t(
                                "Press ~INPUT_PICKUP~ to push the stretcher.\nPress ~INPUT_FRONTEND_RDOWN~ to use the stretcher.",
                                G
                            )
                        end
                    else
                ...
                if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                    ...
                    if H ~= -1 and globalNHSOnDuty then
                        K =
                            "Press ~INPUT_CELLPHONE_CAMERA_EXPRESSION~ to stop pushing the stretcher. Press ~INPUT_CELLPHONE_CAMERA_GRID~ to place the nearest player on."
                        if IsControlJustPressed(0, 183) then
                            local L = GetPlayerServerId(H)
                            local M = NetworkGetNetworkIdFromEntity(d)
                            if L > 0 and M ~= 0 then
                                TriggerServerEvent("XTRA:forcePlayerOnToStretcher", L, M)
                            end
                        end
                    end
                    r(K)
                ...
                if
                    DecorGetBool(c, "has_stretcher_inside") == 1 and globalNHSOnDuty and not IsPedInVehicle(x, c, false) and
                        not b
                 then
                ...
                if
                    IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and
                        not b
                 then
                    if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                        local O = tXTRA.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
                ...
                local N = GetWorldPositionOfEntityBone(c, GetEntityBoneIndexByName(c, "bonnet"))
                if
                    not DecorGetBool(c, "has_stretcher_inside") and
                        IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and
                        not IsPedInVehicle(x, c, false)
                 then
                    DrawMarker(
                        1,
                        N.x,
...
function pushStretcher(x, d)
    Citizen.CreateThread(
        function()
            ...
            TriggerServerEvent("XTRA:stretcherAttachPlayer", P)
            AttachEntityToEntity(
                d,
                PlayerPedId(),
                PlayerPedId(),
            ...
            tXTRA.loadAnimDict("anim@heists@box_carry@")
            local Q = 0
            while IsEntityAttachedToEntity(d, x) do
                ...
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O =
                        tXTRA.getObjectId(
                        DecorGetInt(d, "inside_ambulance_netid"),
                        "pushStretcher(ped, nearbyStretcher)"
                    )
                    if DecorGetBool(O, "has_stretcher_inside") ~= false then
                        if not NetworkHasControlOfEntity(O) then
                            if GetGameTimer() - Q > 200 then
                                TriggerServerEvent(
                                    "XTRA:updateHasStretcherInsideDecor",
                                    tXTRA.getNetId(O, "Stretchers"),
                                    false
                                )
                                Q = GetGameTimer()
                            end
                ...
                if c ~= nil and c ~= 0 then
                    ...
                    if
                        IsControlJustReleased(0, 300) and
                            (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and
                            not R
                     then
                        ClearPedTasksImmediately(x)
                        SetVehicleExtra(d, 1, 1)
                        ...
                        if IsThisModelAHeli(GetEntityModel(c)) then
                            ...
                            AttachEntityToEntity(
                                d,
                                c,
                                GetEntityBoneIndexByName(c, "engine"),
                                0.0,
                                -3.0,
                                -1.0,
                                0.0,
                        ...
                        if not NetworkHasControlOfEntity(d) then
                            TriggerServerEvent(
                                "XTRA:updateStretcherLocation",
                                tXTRA.getNetId(d, "Stretchers"),
                                tXTRA.getNetId(c, "Stretchers")
                            )
                        else
                            DecorSetInt(d, "inside_ambulance_netid", tXTRA.getNetId(c, "Stretchers"))
                        end
...
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            ...
            local g = tXTRA.loadModel("trump-stretcher")
            local D = XTRA.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
            ...
            tXTRA.notify("Stretcher created.")
        end
    end,
    false
)
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            ...
            if V ~= nil then
                TriggerServerEvent("XTRA:removeStretcher", tXTRA.getNetId(V, "Stretchers"))
            end
        end
    end,
    false
...
RegisterNetEvent(
    "XTRA:setStretcherInside",
    function(P, W)
        local D = tXTRA.getObjectId(P, "XTRA:setStretcherInside")
        DecorSetInt(D, "inside_ambulance_netid", W)
    end
)
RegisterNetEvent(
    "XTRA:setHasStretcherInsideDecor",
    function(X, Y)
        local c = tXTRA.getObjectId(X, "XTRA:setHasStretcherInsideDecor")
        DecorSetBool(c, "has_stretcher_inside", Y)
    end
)
RegisterNetEvent(
    "XTRA:toggleAmbulanceDoorStatus",
    function(X)
        local c = tXTRA.getObjectId(X, "XTRA:toggleAmbulanceDoorStatus")
...
function tXTRA.isUsingStretcher()
    return b
end
RegisterNetEvent(
    "XTRA:forcePlayerOnToStretcher",
    function(P)
        if not NetworkDoesNetworkIdExist(P) then
            return
        end
