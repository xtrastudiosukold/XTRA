...
Citizen.CreateThread(
    function()
        while true do
            local e = tXTRA.getPlayerVehicle()
            if e ~= 0 and GetPedInVehicleSeat(e, -1) == tXTRA.getPlayerPed() then
                local f = GetVehicleEngineHealth(e)
                if f <= b and a[e] == nil then
                    local g = GetVehicleClass(e)
                    if g ~= 8 and g ~= 16 and g ~= 15 and g ~= 13 then
                        TriggerServerEvent("XTRA:playNuiSound", "airbag", 15.0, soundEventCode)
                        createAirbags(e)
                    end
                else
                    if a[e] == nil then
                        d = GetPedArmour(tXTRA.getPlayerPed())
                    end
                end
            else
                d = 0
...
function createAirbags(e)
    ...
    if d > GetPedArmour(tXTRA.getPlayerPed()) then
        tXTRA.setArmour(d)
    end
    d = 0
    SetTimeout(
