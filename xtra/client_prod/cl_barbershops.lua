        ...
        if c then
            ...
            local f = function()
                if IsControlJustPressed(1, 51) then
                    TriggerEvent("XTRA:openBarbershop")
                end
            end
            for g, h in pairs(a) do
                tXTRA.createArea("barbershop_" .. g, h, 1.5, 6, d, e, f)
                tXTRA.addMarker(h.x, h.y, h.z - 0.2, 0.5, 0.5, 0.5, 0, 50, 255, 4, 50, 20, false, false, true)
                tXTRA.addBlip(h.x, h.y, h.z, 71, 13, "Barber Shop")
            end
        end
    end
)
