...
function tXTRA.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tXTRA.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tXTRA.cancelOverrideTimeWeather()
    c = false
end
function tXTRA.setWeather(i)
    a = i
end
...
tXTRA.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent(
    "XTRA:setTime",
    function(f, g, h)
        tXTRA.setTime(f, g, h)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("XTRA:vote", C)
            k = true
            tXTRA.notify("~g~Vote sent!")
        else
            tXTRA.notify("You have already voted!")
        end
    else
        tXTRA.notify("Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterCommand(
    "voteweather",
    function()
        TriggerServerEvent("XTRA:tryStartWeatherVote")
    end,
    false
)
RegisterCommand(
