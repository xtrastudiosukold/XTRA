local function a()
    local b = GetResourceKvpString("xtra_scope_horizontal")
    if b == nil or b == "" then
        return 50
    else
        return tonumber(b)
...
local function f()
    local b = GetResourceKvpString("xtra_scope_vertical")
    if b == nil or b == "" then
        return 30
    else
        return tonumber(b)
...
local i = GetResourceKvpString("xtra_scope_enabled") == "true"
local j = false
RMenu.Add(
    "scope",
    "main",
                ...
                function()
                    ...
                    RageUI.List(
                        ...
                        function(k, l, m, n)
                            d = n
                            if l then
                                SetResourceKvp("xtra_scope_horizontal", tostring(n))
                            end
                        end
                    )
                    RageUI.List(
                        ...
                        function(k, l, m, n)
                            h = n
                            if l then
                                SetResourceKvp("xtra_scope_vertical", tostring(n))
                            end
                        end
                    )
                    RageUI.Checkbox(
                        ...
                        function(k, m, l, o)
                            i = o
                            if l then
                                SetResourceKvp("xtra_scope_enabled", tostring(o))
                            end
                        end
                    )
                end
...
function tXTRA.doesCurrentWeaponHaveScope()
    local p = PlayerPedId()
...
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(0)
            if i and (j or tXTRA.doesCurrentWeaponHaveScope()) then
                if j then
                    j = RageUI.Visible(RMenu:Get("scope", "main"))
                end
                local I = math.round(B(), 0)
                DrawAdvancedText(
                    d / 100.0,
                    1.0 - h / 100.0,
                    0.1,
