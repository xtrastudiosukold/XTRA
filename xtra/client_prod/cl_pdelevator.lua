...
RMenu.Add(
    "xtrapdelevators",
    "main",
    RageUI.CreateMenu("XTRA Elevators", "Vespucci PD", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight())
)
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("frpdelevators", "main")) then
            RageUI.DrawContent(
...
AddEventHandler(
    ...
    function(i, j)
        if j then
            local k = function()
                RageUI.Visible(RMenu:Get("xtrapdelevators", "main"), true)
            end
            local l = function()
                RageUI.Visible(RMenu:Get("xtrapdelevators", "main"), false)
            end
            local m = function()
                RMenu:Get("xtrapdelevators", "main"):SetSubtitle(string.format("~w~You are on ~b~%s", a[b].name))
            end
            for n, o in pairs(a) do
                tXTRA.createArea("pdelevator_" .. n, o.coords, 1.5, 6, k, l, m)
                tXTRA.addMarker(
                    o.coords.x,
                    o.coords.y,
                    o.coords.z - 0.98,
