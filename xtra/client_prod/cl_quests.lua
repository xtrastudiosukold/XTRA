...
local function e()
    tXTRA.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
    TaskPlayAnim(
        PlayerPedId(),
...
local function i()
    ...
    if j == 0 then
        a = true
        SetResourceKvpInt("xtra_quests_completedall", 1)
        TriggerServerEvent("XTRA:claimQuestReward")
    end
    return a
end
local function l()
    hasCompletedAllQuestsInt = GetResourceKvpInt("xtra_quests_completedall")
    if not hasCompletedAllQuestsInt then
        SetResourceKvpInt("xtra_quests_completedall", 0)
    end
    if hasCompletedAllQuestsInt == 0 then
        for m = 1, 50 do
            local n = GetResourceKvpInt("xtra_quests_q" .. m .. "_status")
            if not n then
                SetResourceKvpInt("xtra_quests_q" .. m .. "_status", 0)
                n = 0
            end
            if n == 0 then
                c[m] = true
...
local function o(p)
    c[p] = nil
    SetResourceKvpInt("xtra_quests_q" .. p .. "_status", 1)
    TriggerServerEvent("XTRA:setQuestCompleted")
    i()
end
local function q()
    local j = 50
    ...
    function()
        ...
        for w, k in pairs(c) do
            ...
            local x = function(y)
                if not b[y.undiscoveredQID] then
                    if not DoesEntityExist(b[y.undiscoveredQID]) then
                        b[y.undiscoveredQID] =
                            CreateObject(
                            y.undiscoveredQ.model,
                            y.undiscoveredQ.pos.x,
                            y.undiscoveredQ.pos.y,
                            y.undiscoveredQ.pos.z,
                            false,
                            true,
                            false
                        )
            ...
            tXTRA.createArea(
                ...
                z,
                A,
                {undiscoveredQID = w, undiscoveredQ = undiscoveredQ}
            )
            local B = function()
            end
            local C = function()
            ...
            local A = function(y)
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if q() ~= 50 then
                        ...
                        DoScreenFadeIn(2000)
                        tXTRA.removeArea("quests_prop_id_" .. y.undiscoveredQID)
                        tXTRA.removeArea("quests_text_id_" .. y.undiscoveredQID)
                    else
                        r("~g~+14 Days XTRA Club Platinum", "You collected them all!", 600)
                        PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
                        ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE", 1.0)
            ...
            tXTRA.createArea(
                ...
                C,
                A,
                {undiscoveredQ = undiscoveredQ, undiscoveredQID = w}
            )
        end
    end
)
