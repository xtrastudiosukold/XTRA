...
RegisterNetEvent(
    "XTRA:playGrindingScenario",
    function(c, d)
        if not a then
            a = true
            local e = GetGameTimer()
            TaskStartScenarioInPlace(tXTRA.getPlayerPed(), c, 0, true)
            local f
            if d then
...
RegisterNetEvent(
    ...
    function()
        if not a then
            ...
            local j = tXTRA.loadModel("prop_tool_pickaxe")
            ...
            tXTRA.notify("Mining currently in progress.")
        end
    end
)
local l = false
            ...
            local u = function(s)
                if s.nearby then
                    if IsControlJustReleased(0, 38) and GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if not a then
                            m()
                            TriggerServerEvent("XTRA:requestGrinding", s.drug, s.type)
                            Wait(500)
                        else
                            tXTRA.notify("Action in progress, please wait.")
                        end
                    end
                end
            end
...
Citizen.CreateThread(
    function()
        ...
        local A = tXTRA.loadModel("a_c_hen")
