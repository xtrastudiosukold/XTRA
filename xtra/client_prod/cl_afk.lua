                ...
                if h == prevPos and i == previousHeading and j == prevCameraHeading then
                    if d > 0 then
                        if k and d < 1500 and not e then
                            e = true
                            TriggerServerEvent("XTRA:factionAfkAlert", "5 minutes")
                        end
                        if d == 1202 then
                            e = false
                            if not e and k then
                                TriggerServerEvent("XTRA:factionAfkAlert", "10 minutes")
                                e = true
                            end
                        end
                        if b and d == math.ceil(a / 2) then
                            tXTRA.notify(string.format("~r~You will be set to AFK in: %s seconds", d))
                        end
                        ...
                        if not f then
                            if k then
                                TriggerServerEvent("XTRA:factionAfkAlert", "30 minutes - Player set to AFK")
                            end
                    ...
                    if e and k then
                        TriggerServerEvent("XTRA:factionAfkAlert", "No longer AFK - Player moved again")
                        e = false
                    end
                    if f then
                        TriggerServerEvent("XTRA:setNoLongerAFK")
                    end
                    c = false
                    d = a
                    f = false
