...
local f = 0
local g = false
RMenu.Add("identity", "main", RageUI.CreateMenu("", "City Hall", 0, 100, "xtra_cityhallui", "xtra_cityhallui"))
        ...
        if true then
            ...
            local f = function()
                if IsControlJustPressed(1, 51) then
                    TriggerServerEvent("XTRA:getIdentity")
                    RageUI.ActuallyCloseAll()
                    RageUI.Visible(RMenu:Get("identity", "main"), not RageUI.Visible(RMenu:Get("identity", "main")))
                end
            end
            for d, e in pairs(a) do
                tXTRA.createArea("identity_" .. d, e, 1.5, 6, h, i, f)
                tXTRA.addMarker(e.x, e.y, e.z - 0.2, 0.5, 0.5, 0.5, 0, 50, 255, 4, 50, 20, false, false, true)
            end
        end
    end
)
RegisterNetEvent("XTRA:gotCurrentIdentity")
AddEventHandler(
    "XTRA:gotCurrentIdentity",
    function(j, k, l)
        d = j
        e = k
        f = l
...
RegisterNetEvent("XTRA:gotNewIdentity")
AddEventHandler(
    "XTRA:gotNewIdentity",
    function(j, k, l)
        newfirstname = j
                    ...
                    if d ~= "N/A" then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(m, n, o)
                                if o then
                                    RageUI.ActuallyCloseAll()
                                    TriggerServerEvent("XTRA:getNewIdentity")
                                end
                            end
                        )
                    end
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(d, e, m)
                            if m then
                                TriggerServerEvent("XTRA:ChangeIdentity", newfirstname, newlastname, tonumber(newage))
                                RageUI.ActuallyCloseAll()
                            end
                        end,
...
RegisterNetEvent(
    ...
    function(w, x, y, z, A, B, C, D, E)
        ...
        local I = tXTRA.getFontId("Akrobat-ExtraLight")
        y = string.upper(y)
        z = string.upper(z)
        while p do
