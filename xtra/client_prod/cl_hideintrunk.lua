...
local j = false
function tXTRA.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tXTRA.isHandcuffed() or tXTRA.isTazed() then
        return false
    end
    ...
    if l and #(tXTRA.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local q = tXTRA.getPlayerVehicle()
            local r = tXTRA.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(tXTRA.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    local u = GetEntityBoneIndexByName(r, "boot")
                    local v = GetWorldPositionOfEntityBone(r, u)
                    local w = #(v - tXTRA.getPlayerCoords())
                    if w < 2 then
                        DrawMarker(
                            0,
                            v.x,
                            ...
                            if not b(vehicle) then
                                ...
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        ...
                                        tXTRA.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        tXTRA.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        TaskPlayAnim(
                                            p,
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    tXTRA.notify("Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        tXTRA.notify("Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    tXTRA.notify("You and the vehicle must remain stationary to get in.")
                                end
                            else
                                tXTRA.notify("Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
