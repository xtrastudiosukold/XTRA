...
local function r(s, t)
    ...
    while #(tXTRA.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tXTRA.loadModel("metrotrain")
    tXTRA.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function N()
    TriggerEvent("XTRA:Announce", "FOLLOW THE YELLOW MARKERS!")
...
local function O()
    local P = tXTRA.addMarker(b.x, b.y, b.z, 1.0, 1.0, 1.0, 50, 205, 50, 150, 50, 32, false, false, true)
    ...
    ScaleformMovieMethodAddParamTextureNameString("XTRA TUTORIAL")
    ScaleformMovieMethodAddParamTextureNameString("~g~Welcome to XTRA")
    ...
    local E = PlayerPedId()
    while true do
        local R = #(tXTRA.getPlayerCoords() - b)
        if R < 3.0 then
    ...
    tXTRA.removeMarker(P)
    SetScaleformMovieAsNoLongerNeeded(Q)
    tXTRA.loadAnimDict("cellphone@")
    tXTRA.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tXTRA.getPlayerCoords()
...
local function V()
    ...
    while tXTRA.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tXTRA.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    while tXTRA.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tXTRA.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tXTRA.getPlayerVehicle() == 0 do
        if RageUI.Visible(RMenu:Get("XTRAGarages", "main")) then
            ...
            h = "Select Garages"
        elseif RageUI.Visible(RMenu:Get("XTRAGarages", "owned_vehicles")) then
            i = "This lists all your garage types. For now you will only have the Standard Garage."
            h = "Select Standard Garage"
        elseif RageUI.Visible(RMenu:Get("XTRAGarages", "owned_vehicles_submenu")) then
            i = "This lists all the vehicles you have bought for this garage type."
            h = "Select your newly purchased vehicle"
        elseif RageUI.Visible(RMenu:Get("XTRAGarages", "owned_vehicles_submenu_manage")) then
            i = "You can spawn your vehicle, or choose to sell and rent it to another player here."
            h = "Press Spawn Vehicle"
        elseif RageUI.Visible(RMenu:Get("XTRAGarages", "settings")) then
            i = nil
            h = "Go back to the main menu"
        elseif
            RageUI.Visible(RMenu:Get("XTRAGarages", "rented_vehicles")) or
                RageUI.Visible(RMenu:Get("XTRAGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("XTRAGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tXTRA.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tXTRA.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a6()
    ...
    local a7 = tXTRA.getCustomization()
    local a8 = false
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("XTRAclothing") then
            if a8 then
                break
            end
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tXTRA.getPlayerCoords()) < 2.5
            if a2 then
            ...
            if RageUI.Visible(RMenu:Get("XTRAclothing", "mainMenu")) then
                ...
                h = "Select Clothes"
            elseif RageUI.Visible(RMenu:Get("XTRAclothing", "changeClothing")) then
                i = "You can pick from an arrangement of stock and custom clothing. Scroll through to see each item."
                h = "Change your outfit"
            elseif RageUI.Visible(RMenu:Get("XTRAclothing", "changePed")) then
            ...
            elseif RageUI.Visible(RMenu:Get("XTRAclothing", "clearProps")) then
                i = "This can be used to remove accessories. You can also toggle them through the Y menu."
                h = "Go back to pick an outfit"
            end
            if not table.contentEquals(tXTRA.getCustomization(), a7) then
                a8 = true
            end
        end
        Citizen.Wait(0)
...
local function ac()
    ...
    EndScaleformMovieMethod()
    p("questcomplete")
    TriggerServerEvent("XTRA:setCompletedTutorial")
    local U = GetGameTimer()
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tXTRA.isDev() then
        if m > 20000 then
            a1()
        end
    else
...
RegisterNetEvent("XTRA:playTutorial", ad)
AddEventHandler(
    "XTRA:onClientSpawn",
    function(ae, af)
        if af and not tXTRA.isPurge() then
            Citizen.Wait(10000)
            TriggerServerEvent("XTRA:checkTutorial")
        end
    end
)
local function ag()
    if g then
        ...
        if not tXTRA.isPurge() then
            ...
            tXTRA.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tXTRA.createThreadOnTick(ag)
RegisterNetEvent(
    "XTRA:setIsNewPlayer",
    function()
        f = true
...
function tXTRA.isNewPlayer()
    return f
end
function tXTRA.isInTutorial()
    return g
end


function tXTRA.OpenTutorialMenu()
    if tXTRA.isInTutorial() then
        FreezeEntityPosition(PlayerPedId(), true)
        tXTRA.setCustomization(getDefaultCustomization(false), true, true)
...
RegisterNUICallback(
    "PickGender",
    function(au)
        tXTRA.setCustomization(getDefaultCustomization(tobool(au.gender)), true, true)
        tXTRA.showUI()
