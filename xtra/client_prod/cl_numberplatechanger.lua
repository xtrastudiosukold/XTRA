...
local d = module("xtra-vehicles", "cfg/cfg_garages")
d = d.garages
RMenu.Add("plateshop", "main", RageUI.CreateMenu("", "DVLA", 1350, 50, "xtra_licenseplateui", "xtra_licenseplateui"))
RMenu.Add(
    "plateshop",
    "ownedvehicles",
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(e, f, g)
                            if g then
                                ...
                                if h ~= "" then
                                    TriggerServerEvent("XTRA:checkPlateAvailability", h)
                                end
                            end
                        end
                    )
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.Button(
                        ...
                        function(e, f, g)
                            if g then
                                TriggerServerEvent("XTRA:ChangeNumberPlate", selectedCar)
                            end
                        end
                    )
                end
        ...
        if i then
            ...
            local d = function(k)
                if IsControlJustPressed(1, 38) then
                    TriggerServerEvent("XTRA:getCars")
                    RageUI.ActuallyCloseAll()
                    RageUI.Visible(RMenu:Get("plateshop", "main"), not RageUI.Visible(RMenu:Get("plateshop", "main")))
                end
                tXTRA.DrawText3D(c, "Press [E] to open License Plate Management", 0.4)
            end
            tXTRA.createArea("licenseplate", c, 1.5, 6, j, n, d, {})
            tXTRA.addMarker(c.x, c.y, c.z - 1, 1.0, 1.0, 1.0, 255, 0, 0, 4, 50, 27)
            tXTRA.addBlip(c.x, c.y, c.z, 606, 2, "Licence Plate Manager")
        end
    end
)
RegisterNetEvent("XTRA:RecieveNumberPlate")
AddEventHandler(
    "XTRA:RecieveNumberPlate",
    function(p)
        a = p
        RageUI.ActuallyCloseAll()
...
RegisterNetEvent("XTRA:carsTable")
AddEventHandler(
    "XTRA:carsTable",
    function(q)
        b = q
