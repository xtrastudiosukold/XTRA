local a = {
    ...
    [74] = {
        id = 74,
        position = vector3(-2171.3430175781,5140.5693359375,2.8200018405914),
        name = "VIP Island",
        destination = {75, 76},
    },
    [75] = {
...
RMenu.Add(
    "vkint",
    "mainmenu",
    RageUI.CreateMenu("Teleporter", "Main Menu", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight())
)
local b = nil
local function c(d)
    if tXTRA.getPlayerCombatTimer() > 0 then
        tXTRA.notify("You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(
    function()
        for f, d in pairs(a) do
            tXTRA.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
            tXTRA.createArea(
                "vkint_" .. d.id,
                d.position,
                2.0,
