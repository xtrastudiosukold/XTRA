RMenu.Add(
    "cardev",
    "mainmenu",
    RageUI.CreateMenu("", "", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight(), "banners", "cardeveloper")
)
    ...
    function()
        if RageUI.Visible(RMenu:Get("cardev", "mainmenu")) then
            RageUI.DrawContent(
                ...
                function()
                    if b and tXTRA.getPlayerBucket() == 333 then
                            ...
                            function(h, i, j)
                                if j then
                                    tXTRA.clientPrompt(
                                        ...
                                        "",
                                        function(k)
                                            if k ~= "" and tXTRA.getPlayerBucket() == 333 then
                                                local l = tXTRA.loadModel(k)
                                                TriggerServerEvent("XTRA:logVehicleSpawn", k, "/cardev")
                                                ...
                                                SetPedIntoVehicle(tXTRA.getPlayerPed(), n, -1)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(h, i, j)
                                if j then
                                    local o = GetVehiclePedIsIn(tXTRA.getPlayerPed(), false)
                        ...
                        RageUI.Checkbox(
                            ...
                            function()
                                b = true
                                TriggerServerEvent("XTRA:setCarDevMode", b)
                            end,
                            function()
                                b = false
                                TriggerServerEvent("XTRA:setCarDevMode", b)
            ...
            RageUI.DrawContent(
                ...
                function()
                    for w, x in pairs(f) do
                        if GetNumVehicleMods(tXTRA.getPlayerVehicle(), w) > 0 then
                            RageUI.ButtonWithStyle(
                                x,
                                "",
                                {RightLabel = "→→→"},
        ...
        if RageUI.Visible(RMenu:Get("cardev", "vehiclemodindexes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    if GetNumVehicleMods(tXTRA.getPlayerVehicle(), selectedModType) == 0 then
                        RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
                    else
                        for w = 0, GetNumVehicleMods(tXTRA.getPlayerVehicle(), selectedModType) do
                            RageUI.ButtonWithStyle(
                                "Mod " .. w,
                                "",
                                {RightLabel = "→→→"},
                                ...
                                function(h, i, j)
                                    if j then
                                        SetVehicleModKit(tXTRA.getPlayerVehicle(), 0)
                                        SetVehicleMod(tXTRA.getPlayerVehicle(), selectedModType, w)
                                    end
                                end,
                                RMenu:Get("cardev", "vehiclemodindexes")
                            )
        ...
        if RageUI.Visible(RMenu:Get("cardev", "extras")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    local r = tXTRA.getPlayerVehicle()
                    local y = false
                    if r ~= 0 then
                        for w = 1, 12 do
                                    ...
                                    RageUI.Button(
                                        ...
                                        function(h, i, j)
                                            if j then
                                                ...
                                                TriggerEvent("XTRA:staffFixVehicle")
                                            end
                                        end
                                    )
                                end
                ...
                function()
                    for z, A in pairs(g) do
                        RageUI.Button(
                            ...
                            function(h, i, j)
                                if j then
                                    SetVehicleColours(tXTRA.getPlayerVehicle(), A.colorindex, A.colorindex)
                                end
                            end
                        )
                    end
...
RegisterCommand(
    "cardev",
    function(C)
        if a and not tXTRA.isPurge() then
            B()
        end
    end
)
...
local function ae()
    if not b or tXTRA.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tXTRA.getFontId("Akrobat-ExtraBold")
    ...
    local t = tXTRA.getPlayerVehicle()
