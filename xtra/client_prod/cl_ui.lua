...
local l = 10.0
local m = {0, 0, 0, 255}
function tXTRA.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tXTRA.getCachedMinimapAnchor()
    return k
end
function tXTRA.getCachedResolution()
        ...
        while true do
            if not globalHideUi then
                if not globalHideEmergencyCallUI then
                    ...
                    local E = tXTRA.getShowHealthPercentageFlag()
                    if C > 1.0 then
                        C = 1.0
                    end
