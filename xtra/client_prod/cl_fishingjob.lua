            ...
            ontick_fishing = function(y)
                if globalFishingOnDuty then
                    if
                     ...
                     then
                        TriggerServerEvent("XTRA:fishingJobChecks", y.type)
                    end
                    if IsControlJustPressed(0, 47) then
                        local z = GetVehiclePedIsIn(PlayerPedId(), true)
                        tXTRA.notify("~y~Boat anchor set to " .. tostring(not x))
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("XTRA:fishingCaughtFish", d)
            else
                TriggerServerEvent("XTRA:fishingCaughtFish", d)
...
tXTRA.createThreadOnTick(func_fishingStatus)
RegisterNetEvent(
    "XTRA:fishingJobSetCasted",
    function()
        g = true
...
tXTRA.createThreadOnTick(func_createBar)
