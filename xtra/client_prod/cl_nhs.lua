RMenu.Add(
    "nhs",
    "main",
    RageUI.CreateMenu("NHS", "Status: ~b~Available", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight())
)
RMenu.Add(
    "nhs",
    "equipment",
                    ...
                    if e.active then
                        ...
                        if e.readyToRevive then
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("XTRA:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("XTRA:playNhsSound", "shockAdvisedCharging")
                                    end
                                end
                            )
                        else
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        ...
                                        TriggerServerEvent("XTRA:playNhsSound", "threeBeeps")
                                    end
                                end
                            )
                        end
            ...
            RageUI.DrawContent(
                ...
                function()
                            ...
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("XTRA:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("XTRA:playNhsSound", "carryOutShock")
                                    end
                                end
                            )
                        end
                    ...
                    RageUI.Button(
                        ...
                        function(g, h, i)
                            if i then
                                TriggerServerEvent("XTRA:playNhsSound", "metronome")
                            end
                        end
                    )
...
function carryLifePak()
    ...
    f.active = true
    local l = tXTRA.loadModel("prop_lifepak")
    ...
    Citizen.CreateThread(
        function()
            ...
            if not e.dead then
                ...
                if n == 3 then
                    Wait(3000)
                    TriggerServerEvent("XTRA:playNhsSound", "checkPatientAlarm")
                end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("XTRA:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("XTRA:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                TriggerServerEvent("XTRA:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("XTRA:attachLifepakServer")
end
RegisterNetEvent(
    "XTRA:clientPlayNhsSound",
    function(x, y)
...
RegisterNetEvent(
    ...
    function()
        ...
        e.coma = false
        Wait(3000)
        TriggerServerEvent("XTRA:playNhsSound", "safeToTouch")
        Wait(12000)
        TriggerServerEvent("XTRA:playNhsSound", "threeBeeps")
    end
)
...
RegisterNetEvent(
    ...
    function()
        F = true
        tXTRA.loadAnimDict("missheistfbi3b_ig8_2")
        TaskPlayAnim(
            PlayerPedId(),
        ...
        while F do
            if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
                F = false
                TriggerServerEvent("XTRA:cancelCPRAttempt")
                break
            end
            Citizen.Wait(0)
        end
...
RegisterNetEvent(
    "XTRA:cancelCPRAttempt",
    function()
        F = false
        if tXTRA.canAnim() then
