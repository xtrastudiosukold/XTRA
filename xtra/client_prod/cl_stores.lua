...
RMenu.Add(
    "XTRAStores",
    "confirm",
    RageUI.CreateSubMenu(RMenu:Get("XTRAStores", "main", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight()))
)
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("XTRAStores", "main")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    for b, c in pairs(a.shopItems) do
                        RageUI.Button(
                            c.name,
                            nil,
...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("XTRAStores", "confirm")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.Separator(
            ...
            for b, c in pairs(a.shops) do
                ...
                if isInArea(c, 1.0) and j == false then
                    ...
                    if IsControlJustPressed(0, 51) then
                        ...
                        RageUI.Visible(RMenu:Get("XTRAStores", "main"), true)
                        cLoaction = c
                    end
                end
