...
local function n(j, o)
    ...
    for p, q in pairs(a.rewards) do
        if j >= q.day then
            if q.money then
                f = "£" .. getMoneyStringFormatted(q.item)
            else
                f = tostring(q.name)
            end
        end
        ...
        if RageUI.Visible(RMenu:Get("loginrewards", "mainmenu")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Button(
                        string.format("Day %d", t),
                        g(s),
                        {RightLabel = n(t, not s)},
                        s,
                        function(p, p, u)
                            if u then
                                TriggerServerEvent("XTRA:claimNextLoginReward")
                                b = b + 1
                    ...
                    for j = t + 1, t + v do
                        RageUI.Button(
                            string.format("Day %d", j),
                            i(j, s),
                            {RightLabel = n(j, false)},
                            false,
                            function()
                            end
                        )
...
RegisterNetEvent(
    "XTRA:setDailyRewardInfo",
    function(w, x, y)
        b = w
        c = GetGameTimer() + (86400 - (y - x)) * 1000
        if w == 0 then
            Citizen.Wait(20000)
            tXTRA.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
        end
    end
)
RegisterCommand(
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if r() and b > 0 then
                tXTRA.notify("~g~You can claim your next daily reward at /dailyrewards!")
                break
            end
            Citizen.Wait(6000)
        end
