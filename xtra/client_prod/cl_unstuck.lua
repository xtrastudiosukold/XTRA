    ...
    Citizen.CreateThread(
        function()
            ...
            if not g then
                ...
                TriggerServerEvent("XTRA:unstuckSuccessful", d, e)
            else
                tXTRA.notifyPicture(
                    "CHAR_BLOCKED",
                    "CHAR_BLOCKED",
...
RegisterCommand(
    ...
    function(q, r, s)
        local h = PlayerPedId()
        local t, p = o()
        if not t or tXTRA.getPlayerCombatTimer() > 0 then
            tXTRA.notifyPicture(
                "CHAR_BLOCKED",
                "CHAR_BLOCKED",
            ...
            tXTRA.loadAnimDict("timetable@amanda@ig_4")
            TaskPlayAnim(h, "timetable@amanda@ig_4", "ig_4_base", 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
            RemoveAnimDict("timetable@amanda@ig_4")
            Wait(1000)
            b(45, tXTRA.getPlayerCoords(), p)
        end
    end
)
local u = 0
