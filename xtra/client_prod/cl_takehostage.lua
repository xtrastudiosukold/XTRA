    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tXTRA.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("XTRA:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent(
    "XTRA:takeHostageReleaseHostage",
    function()
        a.InProgress = false
        a.type = ""
        DetachEntity(PlayerPedId(), true, false)
        ...
        while true do
            if a.type == "agressor" then
                ...
                if IsEntityDead(PlayerPedId()) then
                    ...
                    RemoveAnimDict("reaction@shove")
                    TriggerServerEvent("XTRA:takeHostageReleaseHostage", a.targetSrc)
                end
                ...
                elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                    ...
                    RemoveAnimDict("anim@gangops@hostage@")
                    TriggerServerEvent("XTRA:takeHostageKillHostage", a.targetSrc)
                    TriggerServerEvent("XTRA:takeHostageStop", a.targetSrc)
                    Wait(100)
...
function tXTRA.takeHostageInProgress()
    return a.InProgress
end
