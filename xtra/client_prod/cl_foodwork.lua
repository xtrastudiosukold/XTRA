    ...
    ["burger"] = {
        {
            ...
            ingredients = {
                ...
                ["lettuce"] = {
                    position = vector3(-1199.9595947266, -896.72601318359, 13.859054756165),
                    name = "Lettuce"
                },
        ...
        if f then
            ...
            local i = function(j)
                if not c and IsControlJustPressed(1, 51) then
                    TriggerServerEvent("XTRA:requestStartCooking", j.recipe)
            ...
            for k, l in pairs(a) do
                for m, n in pairs(l) do
                    tXTRA.createArea(
                        ...
                        h,
                        i,
                        {recipe = k, recipeTable = a[k], index = m}
                    )
                    tXTRA.addMarker(
                        n.cookingPosition.x,
                        n.cookingPosition.y,
                        n.cookingPosition.z - 0.2,
                        0.5,
                        0.5,
                        0.5,
                        0,
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tXTRA.loadModel(s.model)
    ...
    function(e, f)
        if f then
            ...
            for w, x in pairs(b) do
                tXTRA.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
            end
        end
    end
)
...
local function D(E)
    ...
    tXTRA.minigameCircularProgressBar(
        {Difficulty = E, Timeout = 100000, onComplete = function(H)
                if H then
                    G = true
                else
                    G = false
...
function cookBBQ()
    ...
    while L < 3 do
        ...
        if H then
            ...
            tXTRA.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tXTRA.startCircularProgressBar(
            "",
    ...
    tXTRA.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tXTRA.getPlayerPed()
    local N = tXTRA.getPlayerCoords()
    local u = tXTRA.loadModel("prop_food_bs_tray_02")
    local O = CreateObject(u, N.x, N.y, N.z, true, true, true)
    AttachEntityToEntity(
        O,
        C,
        GetPedBoneIndex(C, 60309),
        0.17,
        -0.0,
        0.21,
        -52.0,
    ...
    Citizen.CreateThread(
        function()
            while tXTRA.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tXTRA.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 4, 50, 0, false, true, true)
    local X = tXTRA.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tXTRA.createArea(
        "temp_foodorder_complete",
        V,
        1.5,
        6.0,
        function()
            tXTRA.removeArea("temp_foodorder_complete")
            tXTRA.removeMarker(W)
            tXTRA.removeBlip(X)
...
RegisterNetEvent(
    "XTRA:cookingInstruction",
    function(Z)
        for r, _ in pairs(R) do
            tXTRA.removeArea("temp_cooking_instruction")
            tXTRA.removeMarker(_)
        end
        for r, Y in pairs(S) do
            tXTRA.removeBlip(Y)
        end
        ...
        local W = tXTRA.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 4, 50, 0, false, true, true)
        table.insert(R, W)
        local X = tXTRA.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
        SetBlipFlashes(X, true)
        table.insert(S, X)
        local g = function()
        end
        ...
        local i = function()
            if IsControlJustPressed(1, 51) then
                ...
                if Z == "bbq" then
                    ...
                    T(
                        function()
                            TriggerEvent("XTRA:playItemBoughtSound")
                            TriggerServerEvent("XTRA:pickupCookingIngredient", c, Z)
                            tXTRA.setCanAnim(true)
                            tXTRA.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                        end
                    )
                else
                    tXTRA.removeMarker(W)
        ...
        tXTRA.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
    end
)
RegisterNetEvent(
    "XTRA:beginCooking",
    function(a0)
        TriggerEvent("XTRA:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
    end
)
RegisterNetEvent(
    "XTRA:finishedCooking",
    function()
        c = nil
    end
)
