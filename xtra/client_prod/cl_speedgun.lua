        ...
        if tXTRA.globalOnPoliceDuty() then
            if m[1] ~= nil then
                if tonumber(m[1]) > 100 then
                    d = tonumber(m[1])
                    tXTRA.notify(string.format("~g~Maximum speed set to %smph", tonumber(m[1])))
                else
                    d = 101
                    tXTRA.notify("Minimum speed you can set is 101mph!")
                end
            end
        else
            tXTRA.notify("Speed gun is not enabled!")
        end
    end
)
...
tXTRA.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(
    function()
        while true do
            if tXTRA.globalOnPoliceDuty() and not usingDelgun then
                if GetSelectedPedWeapon(tXTRA.getPlayerPed()) == a then
                    b = true
                else
                    b = false
                end
                if b and tXTRA.getPlayerVehicle() == 0 then
                    local n = tXTRA.getPlayerId()
                         ...
                         then
                            ...
                            if h > d and h > 101 then
                                ...
                                if not j[q] then
                                    ...
                                    TriggerServerEvent(
                                        "XTRA:speedGunFinePlayer",
                                        GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),
                                        h - d
                                    )
                                    Citizen.Wait(3000)
                                else
                                    tXTRA.notify("This vehicle has been fined recently!")
                                end
                            else
                                i = "~w~"
                            end
...
RegisterNetEvent("XTRA:speedGunPlayerFined")
AddEventHandler(
    "XTRA:speedGunPlayerFined",
    function()
