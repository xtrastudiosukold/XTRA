...
RegisterCommand(
    "togglemelee",
    function()
        if tXTRA.getUserId() == 0 or tXTRA.getUserId() == -1 then
            c = not c
            tXTRA.notify("Melee Combat: " .. (c and "~g~Enabled" or "Disabled"))
        end
    end
)
function tXTRA.enablePunching(d)
    c = d
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if
                h == GetHashKey("WEAPON_UNARMED") or h == GetHashKey("WEAPON_STAFFGUN") or
                    GetWeapontypeGroup(h) == GetHashKey("GROUP_MELEE")
             then
    ...
    function(h, i)
        if i then
            ...
            for n, o in pairs(a) do
                tXTRA.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
            end
        end
    end
)
    ...
    function(h, i)
        if i then
            ...
            for n, u in pairs(t) do
                tXTRA.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
                tXTRA.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
            end
        end
    end
)
...
function putGloves()
    ...
    local x = tXTRA.loadModel("prop_boxing_glove_01")
    ...
    ActivatePhysics(A)
    if not w then
        w = tXTRA.getPlayerPed()
    end
    AttachEntityToEntity(
        z,
        w,
        GetPedBoneIndex(w, 0xEE4F),
        0.05,
        0.00,
        0.04,
        00.0,
