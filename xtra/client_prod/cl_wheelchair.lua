RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            local a = tXTRA.loadModel("prop_wheelchair_01")
            local b = CreateObject(a, GetEntityCoords(tXTRA.getPlayerPed()), true)
...
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            local b =
                GetClosestObjectOfType(GetEntityCoords(tXTRA.getPlayerPed()), 10.0, GetHashKey("prop_wheelchair_01"))
...
Citizen.CreateThread(
    function()
        while true do
            ...
            local f = tXTRA.getPlayerVehicle()
            if f == 0 then
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(tXTRA.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(tXTRA.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(tXTRA.getPlayerPed()) then
            DetachEntity(tXTRA.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(tXTRA.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(
                tXTRA.getPlayerPed(),
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(tXTRA.getPlayerPed(), B, C, D)
            A = true
        end
        if A and not NetworkHasControlOfEntity(x) and GetGameTimer() - w >= 200 then
            local E = NetworkGetNetworkIdFromEntity(x)
            if E > 0 then
                TriggerServerEvent("XTRA:syncWheelchairPosition", E, GetEntityCoords(x, true), GetEntityHeading(x))
                w = GetGameTimer()
            end
        end
    end
...
function PickUpWheelchair(x)
    ...
    TriggerServerEvent("XTRA:wheelchairAttachPlayer", F)
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, tXTRA.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, tXTRA.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(tXTRA.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(
                tXTRA.getPlayerPed(),
