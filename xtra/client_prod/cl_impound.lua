...
Citizen.CreateThread(
    function()
        for d, e in pairs(a.peds) do
            tXTRA.createDynamicPed(e.modelHash, e.position, e.heading, true, e.animDict, e.animName, 30, false, false)
            tXTRA.addBlip(e.position.x, e.position.y, e.position.z, 357, 81, "Vehicle Impound", 0.8, false)
        end
        Wait(2000)
        local f = function()
            TriggerServerEvent("XTRA:getImpoundedVehicles")
            RageUI.Visible(RMenu:Get("xtraimpound", "main"), true)
        end
        local g = function()
        ...
        for d, e in pairs(a.peds) do
            tXTRA.createArea("vehicle_impound_" .. d, e.position, 3.0, 6, f, g, h, {})
        end
    end
)
RMenu.Add(
...
RMenu.Add("xtraimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("xtraimpound", "main")))
RMenu.Add("xtraimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("xtraimpound", "vehicle_information")))
local i = 0
RageUI.CreateWhile(
    1.0,
    true,
                ...
                if c ~= nil and c ~= {} then
                    for p, q in pairs(c) do
                        RageUI.ButtonWithStyle(
                            ...
                            RMenu:Get("xtraimpound", "vehicle_information")
                        )
                    end
                else
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("xtraimpound", "pay_fine_confirmation")
                    )
                end
            end,
            function()
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, d, l)
                            if l then
                                TriggerServerEvent("XTRA:releaseImpoundedVehicle", i.vehicle)
                            end
                        end,
                        RMenu:Get("xtraimpound", "main")
                    )
...
RegisterNetEvent(
    ...
    function(x, u, y, z, A, B, C, D)
        ...
        if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
            tXTRA.loadModel("flatbed")
            tXTRA.loadModel("a_m_m_prolhost_01")
            P = XTRA.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
            ...
            TriggerServerEvent(
                "XTRA:deleteImpoundEntities",
                x,
                NetworkGetNetworkIdFromEntity(G),
                NetworkGetNetworkIdFromEntity(F)
            )
            v = XTRA.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
            ...
            TriggerServerEvent("XTRA:awaitTowTruckArrival", x, R, T)
...
function tXTRA.impoundVehicleOptions(Z, _, x, a0)
    ...
    elseif a.disallowedVehicleClasses[a2] then
        ...
        TriggerServerEvent("XTRA:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("xtraimpound", "reasons"), true)
    end
end
function tXTRA.isVehicleImpounded(v)
    return c[v] ~= nil
end
...
exports("impound", tXTRA.impoundVehicleOptions)
exports("isImpounded", tXTRA.isVehicleImpounded)
