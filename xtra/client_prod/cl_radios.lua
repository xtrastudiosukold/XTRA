...
local b = {}
local c = 0
function tXTRA.isInRadioChannel()
    return c ~= 0
end
local function d()
...
local function t()
    exports["xtra"]:setRadioChannel(0)
    e(false)
    d()
    c = 0
end
...
AddEventHandler(
    ...
    function(n)
        ...
        if n then
            exports["xtra"]:overrideProximityRange(0.75, true)
        else
            exports["xtra"]:clearProximityOverride()
        end
    end
)
RMenu.Add(
    "radios",
    "mainmenu",
    RageUI.CreateMenu("Radios", "Main Menu", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight())
)
RMenu.Add(
    "radios",
    "advancedeffects",
...
local function J(q, s)
    exports["xtra"]:setRadioChannel(q)
    if exports["xtra"]:getRadioVolume() == 0 then
        exports["xtra"]:setRadioVolume(u("radios_volume", 10) * 10)
    end
    e(true)
    g(s.name)
            ...
            function()
                ...
                RageUI.Slider(
                    ...
                    function(S, U, T, V)
                        if V ~= A and V > 0 then
                            A = V
                            exports["xtra"]:setRadioVolume(A * 10)
...
RegisterCommand(
    ...
    function()
        ...
        TriggerEvent("XTRA:showRadioWheel", X)
    end,
    false
)
...
Citizen.CreateThread(
    function()
        exports["xtra"]:setVoiceProperty("micClicks", false)
        exports["xtra"]:setRadioVolume(u("radios_volume", 10) * 10)
        B = CreateAudioSubmix("XTRARadio")
        SetAudioSubmixEffectRadioFx(B, 0)
        SetAudioSubmixEffectParamInt(B, 0, GetHashKey("default"), 1)
        F()
        G()
        AddAudioSubmixOutput(B, 0)
        exports["xtra"]:setEffectSubmix("radio", B)
        while true do
...
RegisterCommand(
    ...
    function()
        if c ~= 0 then
            if exports["xtra"]:getRadioVolume() == 0 then
                exports["xtra"]:setRadioVolume(u("radios_volume", 10) * 10)
                TriggerServerEvent("XTRA:radiosSetIsMuted", false)
            else
                exports["xtra"]:setRadioVolume(0)
                TriggerServerEvent("XTRA:radiosSetIsMuted", true)
            end
        end
    end,
    false
...
AddEventHandler(
    "XTRA:openRadioConfig",
    function()
        RageUI.Visible(RMenu:Get("radios", "mainmenu"), true)
    end
)
function tXTRA.radioUIVisible(Y)
    e(Y)
end
