    ...
    function(H, I, J)
        ...
        if K ~= -1 then
            if GetPlayerFromServerId(H) == PlayerId() then
                ...
                t("me", J, d, tXTRA.getPlayerPed(), 10000)
            elseif
                #(GetEntityCoords(tXTRA.getPlayerPed()) - GetEntityCoords(GetPlayerPed(K))) < 7.01 and
                    HasEntityClearLosToEntity(tXTRA.getPlayerPed(), GetPlayerPed(K), 17) == 1
             then
                if c then
                    TriggerEvent("chatMessage", "", {255, 0, 0}, " ^6 me | " .. I .. "  " .. "^6  " .. J)
                end
