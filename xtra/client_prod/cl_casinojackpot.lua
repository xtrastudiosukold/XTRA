...
RMenu.Add(
    "XTRAjackpot",
    "instructions",
    RageUI.CreateMenu("", "", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight(), "xtra_jackpotui", "xtra_jackpotui")
)
RMenu:Get("XTRAjackpot", "instructions"):SetSubtitle("JACKPOT")
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("XTRAjackpot", "instructions")) then
            RageUI.DrawContent(
                ...
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("XTRAjackpot", "instructions")
                    )
                end
            )
        end
    ...
    function()
        while true do
            if insideDiamondCasino then
                b = 1000
                a = -1
                local y = GetEntityCoords(tXTRA.getPlayerPed())
                for z = 0, 15, 1 do
                    local A = t[z]
                    local B = #(y - A)
                    if B < b then
...
function goToJackpotChair(E)
    ...
    PlayFacialAnim(tXTRA.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
            ...
            while sittingAtInsideTrack do
                ...
                if j then
                    DrawAdvancedText(
                        ...
                        0.0028,
                        0.6,
                        math.floor(k * 1000) / 1000 .. "% chance of winning!",
                        255,
                        255,
                        255,
                        255,
                    ...
                    if u ~= nil then
                        for R, S in pairs(u) do
                            if S.user_id == tXTRA.getUserId() then
                                DrawRect(
                                    0.72,
                                    0.82,
                                    0.115,
                ...
                if h then
                    DrawAdvancedText(
                        ...
                        0.0028,
                        0.7,
                        getMoneyStringFormatted(p) .. " tickets",
                        0,
                        153,
                        255,
                        255,
        ...
        while true do
            if f then
                if IsDisabledControlJustPressed(0, 22) then
                    local T = tXTRA.KeyboardInput("Bet Amount", "", 30)
                    if tonumber(T) then
                        T = tonumber(T)
                        if T > 0 then
                            p = T
                ...
                if IsControlJustPressed(0, 201) then
                    if h then
                        TriggerServerEvent("XTRA:setJackpotBet", p)
                    else
...
function showHowToJackpot(a4)
    ...
    if a ~= -1 then
        RageUI.Visible(RMenu:Get("XTRAjackpot", "instructions"), a4)
    end
end
Citizen.CreateThread(
    function()
        while true do
            if a ~= -1 and b < 2 then
                if IsControlJustPressed(0, 38) then
                    if d[a] == false then
                        TriggerServerEvent("XTRA:requestSitAtJackpot", a)
                    end
                end
            end
            Wait(0)
...
AddEventHandler(
    ...
    function(a5, a6)
        if a6 then
            TriggerServerEvent("XTRA:requestJackpotChairData")
        end
    end
)
RegisterNetEvent(
    "XTRA:sendJackpotChairData",
    function(a7)
        d = a7
    end
)
        ...
        while true do
            if sittingAtInsideTrack and f then
                ...
                if IsControlJustPressed(0, 202) and not g then
                    ...
                    PlayFacialAnim(tXTRA.getPlayerPed(), "exit_left_facial", H)
                    SetTimeout(
                        2500,
                        function()
                    ...
                    f = false
                    showHowToJackpot(false)
                    TriggerServerEvent("XTRA:leaveJackpotChair")
                    e = -1
                    SetTimeout(
                        5000,
                        function()
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            SetScriptGfxDrawBehindPausemenu(1)
            if #u ~= 0 then
                local aq = u[#u].tickets_end
                local ar = 0.1
                local ae = 0.8
                local as = 0.0
                for z = 1, #u, 1 do
                    local at = u[z].tickets_end - u[z].tickets_start + 1
                    local au = at / aq
                    local av = au * ae
                    u[z].centerXPos = ar + ae * as + av / 2
                    u[z].rectLength = av
                    DrawRect(
                        ar + ae * as + av / 2,
                        0.5,
                ...
                if n then
                    DrawAdvancedText(
                        ...
                        0.0028,
                        1.12,
                        n .. " wins with a chance of " .. math.floor(o * 1000) / 1000 .. "%!",
                        255,
                        255,
                        255,
                        255,
                ...
                if r then
                    DrawAdvancedText(
                        ...
                        0.0028,
                        1.12,
                        "Total Pot: " .. getMoneyStringFormatted(r),
                        255,
                        255,
                        255,
                        255,
...
function getTrackGenderAnimDict()
    local aw = GetEntityModel(tXTRA.getPlayerPed())
    if aw == "mp_m_freemode_01" then
...
RegisterNetEvent(
    ...
    function()
        ...
        if e ~= -1 then
            ...
            local J = func_4783(e, 0, H, I)
            local K = func_4781(e, 0, H, I)
            local ay = NetworkCreateSynchronisedScene(J.x, J.y, J.z, 0.0, 0.0, K, 2, 1, 0, 1065353216, 0, 1065353216)
            NetworkAddPedToSynchronisedScene(tXTRA.getPlayerPed(), ay, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
            NetworkStartSynchronisedScene(ay)
            local Q = setupJackpotInstructionalScaleform("instructional_buttons")
            Citizen.CreateThread(
                function()
                    while f do
