...
function tXTRA.isCompassEnabled()
    return showCompass
end
function tXTRA.setCompassEnabled(f)
    showCompass = f
    SetResourceKvp("xtra_compass", tostring(f))
end
Citizen.CreateThread(
    function()
        local g = GetResourceKvpString("xtra_compass") or "false"
        if g == "false" then
            showCompass = false
        else
            showCompass = true
...
local function h(i, j, k, l)
    tXTRA.DrawText(j, k, i, l.size, 0, 0, {l.colour.r, l.colour.g, l.colour.b, l.colour.a}, true)
end
local function m()
    if showCompass then
        ...
        if a.compass.followGameplayCam then
            ...
            o = 360.0 - GetEntityHeading(tXTRA.getPlayerPed())
        end
