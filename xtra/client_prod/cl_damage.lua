...
function GameEventTriggered(i, j)
    if i == "CEventNetworkEntityDamage" then
        ...
        if m and l == PlayerPedId() and IsEntityAPed(k) and k ~= PlayerPedId() then
            TriggerEvent("XTRA:onPlayerKilledPed", k)
        end
        if k == PlayerPedId() then
            local t = tXTRA.getPedServerId(l)
            if n == 0 then
                ...
                function()
                    ...
                    if DecorExistOn(PlayerPedId(), "lasthp") then
                        ...
                        if u < v then
                            if c[r] then
                                if t then
                                    TriggerServerEvent("XTRA:syncEntityDamage", u, v, t, true, m, n)
                                end
                            elseif d[r] then
                                if t then
                                    TriggerServerEvent("XTRA:syncEntityDamage", u, v, t, false, m, n)
                                end
                            end
                        end
                        if m then
        ...
        if
            tXTRA.getDmgIndcator() and (tXTRA.isPlusClub() or tXTRA.isPlatClub()) and IsEntityAPed(k) and
                k ~= PlayerPedId() and
                l == PlayerPedId()
         then
            ...
            local x, y = tXTRA.getDmgIndcator()
            local z = {
                {200, 0, 0, 255},
                {0, 0, 200, 255},
                {0, 200, 0, 255},
            ...
            if x then
                ...
                Citizen.CreateThread(
                    function()
                        while true do
                            if A >= 0 then
                                tXTRA.DrawText3D(vector3(B.x, B.y, B.z + 0.5), w, 0.5, 4, nil, z[y], true, true)
                                A = A - 10
                            end
                            Wait(0)
                        end
...
AddEventHandler(
    ...
    function(C, D, n, E)
        ...
        if D == F and C ~= F and IsEntityAPed(C) and GetEntityHealth(C) > 105 and not IsEntityDead(C) then
            Citizen.Wait(0)
            TriggerEvent("XTRA:onPlayerDamagePed", C)
        end
    end
)
CreateThread(
...
local A = 0
RegisterNetEvent(
    "XTRA:onEntityHealthChange",
    function(G, u, v, H)
        if f and (tXTRA.isPlusClub() or tXTRA.isPlatClub()) then
