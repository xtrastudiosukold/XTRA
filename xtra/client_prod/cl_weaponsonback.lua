        ...
        for c, d in pairs(b.weapons) do
            if not a.weapons[d.hash] then
                if d.class == "SMG" then
                    a.weapons[d.hash] = {
                        bone = 58271,
                        offset = vector3(-0.01, 0.1, -0.07),
                        rotation = vector3(-55.0, 0.10, 0.0),
                        model = GetHashKey(d.model),
                        type = "SMG"
                    }
...
local function g()
    ...
    local i = tXTRA.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tXTRA.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o =
                (not k or n.type ~= "Heavy") and (tXTRA.isEmergencyService() or not isInGreenzone) and
                not tXTRA.isPlayerInAnimalForm()
    ...
    function(C, c, D)
        ...
        if r then
            ...
            f[A] = {
                ...
                playerIndex = -1,
                weapons = E,
                diagonal = Player(A).state.weaponsDiagonal,
                frontAR = Player(A).state.frontAR,
                frontSMG = Player(A).state.frontSMG
            }
        end
    end
)
