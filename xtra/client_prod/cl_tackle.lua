...
function tXTRA.isPedBeingTackled()
    return h
end
RegisterNetEvent(
    "XTRA:playTackle",
    ...
    for p, q in ipairs(GetActivePlayers()) do
        if q ~= PlayerId() then
            ...
            if t ~= "metpd" and t ~= "hmp" and t ~= "npas" then
                ...
                local v = #(u - tXTRA.getPlayerCoords())
                if v < n then
                    n = v
                    o = r
                end
...
function func_tackleManagement()
    if h then
        SetPedToRagdoll(tXTRA.getPlayerPed(), 1000, 1000, 0, 0, 0, 0)
    end
    if globalOnPoliceDuty or w() or tXTRA.isStaffedOn() or globalOnPrisonDuty and globalPlayerInPrisonZone then
        if IsControlPressed(0, a["LEFTSHIFT"]) and IsControlPressed(0, a["G"]) then
            if
                not b and GetGameTimer() - g > 10 * 1000 and GetEntityHealth(PlayerPedId()) > 102 and
                    not tXTRA.isKnockedOut()
             then
                local x = m()
                if x then
                    if
                        not b and not c and not IsPedInAnyVehicle(tXTRA.getPlayerPed()) and
                            not IsPedInAnyVehicle(GetPlayerPed(x))
                     then
                        b = true
                        g = GetGameTimer()
                        TriggerServerEvent("XTRA:tryTackle", x)
                    end
                end
            end
        end
...
tXTRA.createThreadOnTick(func_tackleManagement)
