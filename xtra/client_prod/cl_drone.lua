...
function tXTRA.isPlayerInDrone()
    return c.active
end
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tXTRA.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tXTRA.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tXTRA.getPlayerCoords(), true)
        ...
        tXTRA.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if tXTRA.getPlayerVehicle() == 0 and not tXTRA.isInComa() then
                TriggerServerEvent("XTRA:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                tXTRA.notify("You are not able to use a drone right now.")
            end
        else
            TriggerServerEvent("XTRA:performDroneLog", "Powered off their DJI Inspire Drone")
            deleteDrone()
        end
    end
)
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    tXTRA.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
...
Citizen.CreateThread(
    function()
        while true do
            local p = tXTRA.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        tXTRA.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = tXTRA.getPlayerCoords()
                        AddOwnedExplosion(tXTRA.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if tXTRA.getPlayerVehicle() ~= nil and DoesEntityExist(tXTRA.getPlayerVehicle()) then
                            DeleteEntity(tXTRA.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = tXTRA.loadModel(a)
    local v = tXTRA.loadModel("s_m_m_bouncer_01")
    local e = tXTRA.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = XTRA.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tXTRA.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tXTRA.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tXTRA.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tXTRA.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tXTRA.notify("To remove your drone, use ~b~/drone")
end
function deleteDrone()
    ...
    tXTRA.notify("Drone ~b~removed~w~.")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tXTRA.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tXTRA.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tXTRA.createThreadOnTick(B)
