...
function tXTRA.isPlayerRappeling()
    return k
end
function tXTRA.isPlayerInPoliceHeli()
    return l
end
local r = (a + b) * 0.5
local s = 0
...
local function X(Y)
    ...
    tXTRA.DrawText(0.45, 0.9, "Model: " .. _ .. "\nPlate: " .. a0, 0.55)
end
local function a1()
    while true do
        if l then
            local a2 = tXTRA.getPlayerPed()
            local heli = tXTRA.getPlayerVehicle()
            ...
            while m and not IsEntityDead(a2) and GetVehiclePedIsIn(a2) == heli and IsHeliHighEnough(heli) do
                ...
                if o then
                    ...
                    if ai - p >= 500 then
                        ...
                        TriggerServerEvent("XTRA:updateSpotlight", ak)
                        p = ai
                    end
...
RegisterNetEvent(
    ...
    function(aw, ax)
        ...
        if az == 0 or az == tXTRA.getPlayerPed() then
            return
        end
