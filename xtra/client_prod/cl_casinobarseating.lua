...
function sitDownAtBar(l)
    ...
    tXTRA.loadAnimDict(a)
    followMachineVector = m
    followMachineRotation = n
    TaskGoStraightToCoord(
        tXTRA.getPlayerPed(),
        followMachineVector.x,
        followMachineVector.y,
        followMachineVector.z,
        1.0,
        50000,
        312.2,
        10.0
...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(tXTRA.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
    ...
    function()
        while true do
            if insideDiamondCasino then
                if e then
                    SetPedCapsule(tXTRA.getPlayerPed(), 0.13)
                    if IsControlJustPressed(0, 202) then
                        tXTRA.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                        TaskPlayAnim(
                            tXTRA.getPlayerPed(),
