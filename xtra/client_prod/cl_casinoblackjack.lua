...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(tXTRA.getPlayerPed())
            ...
            tXTRA.createDynamicPed(
                ...
                function(Q)
                    ...
                    SetEntityCoordsNoOffset(
                        Q,
                        G.blackjackTables[O].dealerPos.x,
                        G.blackjackTables[O].dealerPos.y,
                        G.blackjackTables[O].dealerPos.z,
                        0,
                        0,
                        1
                    )
                    ...
                    tXTRA.loadAnimDict(dealerAnimDict)
        ...
        while true do
            ...
            if h then
                if IsDisabledControlJustPressed(0, 22) then
                    local W = tXTRA.KeyboardInput("Bet Amount", "", 30)
                    if tonumber(W) then
                        W = tonumber(W)
                        if W > 0 then
                ...
                if IsControlJustPressed(0, 201) then
                    if tonumber(z) >= 0 then
                        ...
                        elseif isChairHighRollers(F) and z < B then
                            tXTRA.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                        elseif z > 100000000000 then
                            tXTRA.notify("~r~Maximum bet is £100,000,000,000")
                        else
    ...
    function()
        while true do
            if sittingAtBlackjackTable and A then
                SetPedCapsule(tXTRA.getPlayerPed(), 0.2)
                if IsControlJustPressed(0, 202) and not i then
                    ...
                    TaskPlayAnim(tXTRA.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu.Add(
    "xtrablackjack",
    "instructions",
    RageUI.CreateMenu("", "test", 1300, 50, "casinoui_cards_blackjack", "casinoui_cards_blackjack")
)
RMenu:Get("xtrablackjack", "instructions"):SetSubtitle("Blackjack Table")
RMenu.Add(
    "xtrablackjack_high",
    "instructions",
    RageUI.CreateMenu("", "test", 1300, 50, "casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high")
)
RMenu:Get("xtrablackjack_high", "instructions"):SetSubtitle("High Rollers Table")
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("xtrablackjack", "instructions")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.FakeButton(
                        "test",
                        "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.",
                        {RightLabel = "→→→"},
                        true,
                        function(a0, a1, a2)
                        end,
                        RMenu:Get("xtrablackjack", "instructions")
                    )
                end,
                function()
                end
        ...
        if RageUI.Visible(RMenu:Get("xtrablackjack_high", "instructions")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("xtrablackjack_high", "instructions")
                    )
                end,
                function()
                end
...
function showHowToBlackjack(a3)
    RageUI.ActuallyCloseAll()
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get("xtrablackjack", "instructions"), a3)
    else
        RageUI.Visible(RMenu:Get("xtrablackjack_high", "instructions"), a3)
    end
end
...
RegisterNetEvent(
    "Blackjack:syncChipsPropBlackjack",
    function(a9, Y)
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(tXTRA.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(tXTRA.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(tXTRA.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(tXTRA.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = tXTRA.loadModel(nextCard)
...
function drawNativeNotification(bq)
    SetTextComponentFormat("STRING")
    AddTextComponentString(bq)
