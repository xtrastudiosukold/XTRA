RMenu.Add(
    "mainmenu",
    "cinematic",
    RageUI.CreateMenu("", "", tXTRA.getRageUIMenuWidth(), tXTRA.getRageUIMenuHeight(), "new_editor", "r_editor_header")
)
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("XTRA Cinematic Editor")
RMenu.Add(
    "load_scene",
    "cinematic",
...
RMenu:Get("scene_manager", "cinematic"):SetSubtitle("Scene Manager")
RMenu.Add(
    "camera_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("camera_manager", "cinematic"):SetSubtitle("Camera Manager")
RMenu.Add(
...
RMenu:Get("screeneffect_manager", "cinematic"):SetSubtitle("Screen Effect Manager")
RMenu.Add(
    "timecycle_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("timecycle_manager", "cinematic"):SetSubtitle("Timecycle Effect Manager")
RMenu.Add(
    "shake_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("shake_manager", "cinematic"):SetSubtitle("Shake Effect Manager")
RMenu.Add(
    "add_camera",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("add_camera", "cinematic"):SetSubtitle("Add Camera")
RMenu.Add(
...
RMenu:Get("camera_focus_manager", "cinematic"):SetSubtitle("Camera Focus")
RMenu.Add(
...
RMenu:Get("weather_time_manager", "cinematic"):SetSubtitle("Time/Weather Manager")
RMenu.Add(
    "dof_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("dof_manager", "cinematic"):SetSubtitle("Depth of Field Manager")
local function a(b)
...
Citizen.CreateThread(
    function()
        m = json.decode(GetResourceKvpString("xtra_scenedata") or "{}")
    end
)
RegisterNetEvent(
    "XTRA:openCinematicMenu",
    function()
        a(true)
    end
)
function tXTRA.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Scene created & saved.")
end
function tXTRA.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Scene deleted & saved.")
end
function tXTRA.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Camera created & saved.")
end
function tXTRA.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Camera modified & saved.")
end
function tXTRA.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Camera deleted & saved.")
end
function tXTRA.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Transition change saved.")
end
function tXTRA.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Camera Blending change saved.")
end
function tXTRA.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Screeneffect change saved.")
end
function tXTRA.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Timecycle Effect change saved.")
end
function tXTRA.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Timecycle intensity change saved.")
end
function tXTRA.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Shake change saved.")
end
function tXTRA.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Camera type change saved.")
end
function tXTRA.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~Follow player change saved.")
end
function tXTRA.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("xtra_scenedata", json.encode(m))
    tXTRA.notify("~g~FOV change saved.")
end
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    if q and m[n][q] then
                        ...
                        RageUI.ButtonWithStyle(
                            "Transition: " .. m[n][q].transition .. "ms",
                            "",
                            {RightLabel = "→→→"},
                            true,
                            function(a1, a2, a3)
                                if a3 then
                                    ...
                                    if tonumber(T) then
                                        tXTRA.modifyTransition(n, q, T)
                                    else
                                        tXTRA.notify("Failed to change transition time.")
                                    end
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        ...
                        RageUI.List(
                            ...
                            function(a1, a2, a3, a8)
                                if a8 ~= w then
                                    w = a8
                                    tXTRA.modifyCameraType(n, q, j[w])
                                end
                            end,
                            function()
                            end,
                        ...
                        RageUI.List(
                            ...
                            function(a1, a2, a3, a8)
                                if a8 ~= x then
                                    x = a8
                                    tXTRA.modifyCameraBlending(n, q, k[x])
                                end
                            end,
                            function()
                            end,
                            ...
                            function(a1, a2, a3, a8)
                                ...
                                if a8 ~= e then
                                    ...
                                    if m[n][q].timecycleEffect ~= "" then
                                        ...
                                        tXTRA.setTimecycleIntensity(n, q, f[e])
                                    end
                                end
                            end,
                            function()
                        ...
                        RageUI.Separator("No Camera ID selected.")
                    end
                end
            )
        end
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            if a3 then
                                if o then
                                    notify("Can not add a camera whilst you are adding a camera.")
                                    return
                                end
                                previewingCamera = nil
                                t = GetEntityCoords(PlayerPedId())
                    ...
                    for g = 1, #c do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXTRA.modifyScreeneffect(n, q, c[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #d do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXTRA.modifyTimecycleEffect(n, q, d[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #h do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXTRA.modifyShakeEffect(n, q, h[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
        ...
        if RageUI.Visible(RMenu:Get("camera_focus_manager", "cinematic")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Separator("Currently pointed at: " .. m[n][q].focusType .. " : " .. m[n][q].focusData)
                    RageUI.ButtonWithStyle(
                        "Player",
                        "",
                        {RightLabel = "→→→"},
                        ...
                        function(a1, a2, a3)
                            if a3 then
                                ...
                                if a9 ~= "" then
                                    tXTRA.addCameraFocus(n, q, "player", a9)
                                end
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                            ...
                            if a3 then
                                tXTRA.clientPrompt(
                                    ...
                                    function(aa)
                                        ...
                                        tXTRA.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                                    end
                                )
                            end
                        end,
        ...
        if RageUI.Visible(RMenu:Get("weather_time_manager", "cinematic")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    local function a4()
                        B = true
                        tXTRA.overrideTime(C[D], E[F], G[H])
                    end
                    local function a5()
                        B = false
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if (o or p) and noclipActive and tXTRA.getPlayerVehicle() ~= 0 then
                tXTRA.notify("You may not noclip when in a vehicle.")
                tXTRA.toggleNoclip()
            end
                ...
                if a0.focusType ~= "" then
                    if a0.focusType == "player" then
                        ...
                        if aj then
                            ...
                            print("[XTRA Cinematic] Failed to point cam at player, could not get entity.")
                        end
...
Citizen.CreateThread(
    function()
        while true do
            local al, am = tXTRA.getPlayerVehicle()
            ...
            if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
                ...
                if ao ~= -1 then
                    ...
                    if ap > 0 then
                        if not tXTRA.clientGetPlayerIsStaff(tXTRA.clientGetUserIdFromSource(ap)) then
