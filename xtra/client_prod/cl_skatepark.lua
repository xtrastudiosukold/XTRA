            ...
            local h = function(f)
                ...
                if IsControlJustPressed(1, 38) then
                    if GetGameTimer() - b > 60000 then
                        local i = tXTRA.loadModel(a[f.skateparkId].carID)
                        local j =
                            CreateVehicle(
                            i,
                        ...
                        SetPedIntoVehicle(tXTRA.getPlayerPed(), j, -1)
                        SetModelAsNoLongerNeeded(i)
                        b = GetGameTimer()
                    else
                        tXTRA.notify("Please wait before spawning another BMX.")
                    end
                end
            end
            for k, l in pairs(a) do
                tXTRA.createArea("skatepark_" .. k, l.position, 1.5, 6, e, g, h, {skateparkId = k})
                tXTRA.addMarker(
                    l.position.x,
                    l.position.y,
                    l.position.z,
