...
-- it is not recommended to modify the XTRA core files outside the cfg/ directory, create a new resource instead
-- you can load external images/fonts/etc using the NUI absolute path: nui://my_resource/myfont.ttf
-- example, changing the gui font (suppose a xtra_mod resource containing a custom font)
cfg.css = [[
@font-face {
  font-family: "Custom Font";
  src: url(nui://xtra_mod/customfont.ttf) format("truetype");
}
...
-- static menus are menu with choices defined by XTRA.addStaticMenuChoices(name, choices)
